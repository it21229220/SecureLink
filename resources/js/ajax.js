...
// Function to insert a new message via AJAX
function ajaxInsertMessage(){
  ...
  // If the selected encryption method is ElGamal
  if(encMethodId == 3){
    let gamal = new Gamal();
  ...
  xmlhttp.open("POST", "insert_message.php", true); // Open a POST request to insert_message.php
...
//           //console.log('Message Data:', messagesData[i]);
...
//           }
//           let flaggedAsPhishing = messagesData[i][9]; // Assume this is the flagged_as_phishing field
//           console.log('Message:', theMessage, 'Flagged as phishing:', flaggedAsPhishing); // Log for debugging
//           if (flaggedAsPhishing) {
//             theMessage += " <strong>(Warning: This URL is flagged as phishing)</strong>";
//           }
      ...
      if (this.readyState == 4 && this.status == 200) {
          if (this.responseText != "") {
              ...
              for (let i = 0; i < messagesData.length; i++) {
                  ...
                  let flaggedAsPhishing = messagesData[i][9]; // Assume this is the flagged_as_phishing field
                  //console.log('Message:', theMessage, 'Flagged as phishing:', flaggedAsPhishing); // Log for debugging
                  if (messagesData[i][9] == 1) {

                    theMessage += "  (Warning: This URL is flagged as phishing)";

                  } else if (messagesData[i][9] == 0) {
                      if (messagesData[i][2] == 3) {
