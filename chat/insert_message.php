<?php
  ...
  require_once('../server/virustotal_scan.php');
  $db = DB::getInstance();

  // Retrieve POST data sent from the client
  $senderId = $_POST['sid'];
  ...
  $flaggedAsPhishing = isset($_POST['flaggedAsPhishing']) ? $_POST['flaggedAsPhishing'] : 0; // Default to 0 if not set

   // Variables for different encryption methods
  $keyAes = "";
  $cipherAes = "";
  $senderKeyAes = "";
  $receiverKeyAes = "";
  ...
  // Handle ElGamal encryption method
  ...
  // Retrieve the tokens of the sender and receiver
  ...
  // Insert the encrypted message into the chat table
  $query = "INSERT INTO chat (sender_id, receiver_id, message, is_file, file_name, enc_method, s_token, r_token, flagged_as_phishing) VALUES($senderId, $receiverId, '$eMessage', $isFile, '$fileName', $encMethodId, '$senderToken', '$receiverToken', $flaggedAsPhishing)";
  //echo $query;
